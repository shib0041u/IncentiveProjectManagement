...
<form method="post">
						<table class="table">
							...
							<tbody>
								...
								{
								%>
								<input class ="btn btn-sm btn-danger" type= "submit" value="Block" onclick="form.action='blockEmployeeRequest'
												<%session.setAttribute("id",x.getId());%>">
								
								<%}
								else{
								%>
								<input class ="btn btn-sm btn-success" type= "submit" value="unblock" onclick="form.action='activeEmployeeRequest'
												<%session.setAttribute("id",x.getId());%>"><%}
								...
								<td><%=x.getEmail() %></td>
								<td><%=x.getId() %></td>
								<td><input class="btn btn-sm btn-success" type= "submit" value="Approve" onclick="form.action='registerAsEmployeeFromCEO'
												<%session.setAttribute("id",x.getId());%>"></td>
								<td><input class="btn btn-sm btn-danger" type= "submit" value="Reject" onclick="form.action='employeesRequestRemove'
												<%session.setAttribute("id",x.getId());%>"></td>
								</tr>
								<%
								}
