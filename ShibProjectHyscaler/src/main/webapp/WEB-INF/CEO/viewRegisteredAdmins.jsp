...
<div class="table-responsive">
						<table class="table">
							...
							<tbody>
								...
								List<Admin> admin = dao.fetchAllAdmin();
								if(admin!=null)
								for (Admin x : admin) {
								%>
								<tr>
								...
								%>
								<input class ="btn btn-sm btn-success" type= "submit" value="Unblock" onclick="form.action='activeAdmin'
												<%session.setAttribute("id",x.getId());%>"><%}
								...
								<td><%=x.getEmail() %></td>
								<td><%=x.getId() %></td>
								<td><input type= "submit" value="See Employees" onclick="form.action='employeesUnderAdmin'
